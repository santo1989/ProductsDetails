...
if (!is_logged_in()) {
    $_SESSION['error_message'] = 'Please login to edit products.';
    redirect('index.php');
}
...
// Check if user has permission to edit (own product or admin)
if (!is_admin() && $product['Created_By'] != $_SESSION['user_id']) {
    $_SESSION['error_message'] = 'You do not have permission to edit this product.';
    $conn->close();
    redirect('dashboard.php');
}
    ...
    if (empty($product_name)) {
        ...
        if (empty($error)) {
            $stmt = $conn->prepare("UPDATE products SET 
                                    Product_Name = ?, Category = ?, Size = ?, Description = ?, 
                                    Fabrication = ?, Construction = ?, GSM = ?, Finishes = ?, 
