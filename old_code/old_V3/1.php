...
// Helper function to handle case sensitivity for Oracle
function get_column_name($column, $db_type) {
    if ($db_type == 2 || $db_type == 1) { // Oracle
...
if($action=="load_drop_down_sewing_line_floor")
{
	...
	if($prod_reso_allo==1)
	{
		...
		{
			$line='';
			// Use helper function for column name
			$line_col = get_column_name('line_number', $db_type);
			$line_number=explode(",",$row[csf($line_col)]);
...
{		  
	...
	if($wo_company_name==0) $company_name=""; else $company_name=" and b.company_name in($wo_company_name)";//job_no
		...
		{
			$line_sql="SELECT a.id, a.$line_col from prod_resource_mst a,prod_resource_dtls b where a.id=b.mst_id $data_format and a.is_deleted=0 and b.is_deleted=0 $cond group by a.id, a.$line_col order by a.id";
		}
		// echo $line_sql;
            ...
            <div style="width:250px; max-height:300px; overflow-y:auto;" id="scroll_body" >          
				...
				$previous_line_arr=explode(",",$line_id);
				 foreach($line_sql_result as $row)
				 {
					...
					$line_val='';
					$line_col = get_column_name('line_number', $db_type);
					$line_id_arr=explode(",",$row[csf($line_col)]);
					foreach($line_id_arr as $line_id_val)
					{
						if($line_val=="") $line_val=$line_library[$line_id_val]; else $line_val.=','.$line_library[$line_id_val];
					}
					?>
...
if($action=="report_generate")
{
	...
	// Use helper function for column name
	$line_col = get_column_name('line_number', $db_type);
	$prod_reso_arr=return_library_array( "select id, $line_col from prod_resource_mst",'id',$line_col);
